...
public class FirstMainClass extends JFrame {
    ...
    static TransformGroup wholePenguin;
    ...
    private void setPenguinElementsList() {
        nameMap=scene.getNamedObjects();
        //Print elements of your model:
        printModelElementsList(nameMap);
        
        wholePenguin = new TransformGroup();
        transform3D = new Transform3D();
        transform3D.setScale(new Vector3d(0.5,0.5,0.5));
        wholePenguin.setTransform(transform3D);
        root.removeChild(nameMap.get("default"));
        wholePenguin.addChild(nameMap.get("default"));
        wholePenguin.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
        root.addChild(wholePenguin);
    }
    ...
    private void addAppearance(){
        Appearance PenguinAppearance = new Appearance();
        PenguinAppearance.setTexture(getTexture("source_folder\\fur1.png"));
        TextureAttributes texAttr = new TextureAttributes();
        texAttr.setTextureMode(TextureAttributes.COMBINE);
        PenguinAppearance.setTextureAttributes(texAttr);
        PenguinAppearance.setMaterial(getMaterial());
        Shape3D Penguin = nameMap.get("default");
        Penguin.setAppearance(PenguinAppearance);
    }
    ...
    public static void main(String[]args){
        try {
            ...
            AnimationPenguin PenguinMovement = new AnimationPenguin(wholePenguin, transform3D, window);
            window.addKeyListener(PenguinMovement);
